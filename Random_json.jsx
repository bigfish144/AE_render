...
var modifiedParameters = []; // 用于存储修改的参数

function main() {
    var rootComp = app.project.activeItem; // 获取当前活动的合成作为根合成
    ...
    processLayers(rootComp);
    app.endUndoGroup(); 
    alert(seed); // 输出种子值
    // 将修改的参数导出为JSON并保存到本地文件
    var jsonOutput = JSON.stringify({ assets: modifiedParameters }, null, 2);
    alert(jsonOutput); // 确保在定义后使用
    // 确保路径正确
    var desktopPath = Folder.desktop.fsName;
    var filePath = desktopPath + "/modified_parameters.json";
    var file = new File(filePath);
    file.open("w");
    file.write(jsonOutput);
    file.close();
    alert("Modified parameters saved to: " + file.fsName);
}
...
function processLayers(comp) {
    for (var i = 1; i <= comp.layers.length; i++) {
        ...
        if (effectsGroup) {
            randomizeEffects(effectsGroup, layer.name);
        }
...
function randomizeEffects(effectsGroup, layerName) {
    // 遍历图层上的所有效果器
                    ...
                    if (minVal !== -Infinity && maxVal !== Infinity) {
                        ...
                        try {
                            ...
                            modifiedParameters.push({
                                type: "data",
                                layerName: layerName,
                                property: "Effects." + effect.name + "." + param.name,
